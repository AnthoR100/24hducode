...
export default function Dashboard() {
  ...
  const [brokerLog, setBrokerLog] = useState([]);
  ...
  // ─── Broker ──────────────────────────────────────────
  useEffect(() => {
    const socket = ioClient(BROKER_URL, { transports: ["websocket", "polling"] });
    socket.on("connect", () => {
      setBrokerConnected(true);
      addLog("🔌 Broker connecté", "success");
    });
    socket.on("disconnect", () => {
      setBrokerConnected(false);
      addLog("🔌 Broker déconnecté", "error");
    });
    socket.on("broker:event", (event) => {
      const { type, data } = event;
      const time = new Date().toLocaleTimeString("fr-FR");
      if (type === "OFFRE") {
        const offer = data.message || data;
        const entry = {
          id: offer.id,
          resourceType: offer.resourceType || "?",
          quantityIn: offer.quantityIn || 0,
          pricePerResource: offer.pricePerResource || 0,
          owner: offer.owner || { name: "?" },
          time,
        };
        setLiveOffers(prev => {
          const exists = prev.find(o => o.id === entry.id);
          if (exists) return prev.map(o => o.id === entry.id ? entry : o);
          return [entry, ...prev].slice(0, 50);
        });
        setBrokerLog(prev => [...prev.slice(-100), { time, msg: `📦 ${entry.resourceType} x${entry.quantityIn} à ${entry.pricePerResource} OR/u (${entry.owner.name})`, type: "offer" }]);
      } else {
        setBrokerLog(prev => [...prev.slice(-100), { time, msg: `[${type}] ${JSON.stringify(data).slice(0, 120)}`, type: type.includes("SUCCESS") ? "success" : type.includes("FAIL") ? "error" : "info" }]);
      }
    });
    return () => socket.disconnect();
  }, [addLog]);

  const quickBuy = useCallback(async (offerId, quantity) => {
    const d = await api("POST", "/marketplace/purchases", { offerId, quantity });
    if (d) {
      addLog(`📥 Achat OK: ${quantity} unités`, "success");
      setLiveOffers(prev => prev.filter(o => o.id !== offerId));
      loadPlayerDetails();
    }
  }, [api, addLog, loadPlayerDetails]);
  ...
  useEffect(() => {
    (async () => { const n = await loadCellsFromDB(); await loadDbStats(); addLog(`🗄 ${n} cellules chargées depuis la DB`, "success"); })();
  ...
  const findBestPath = useCallback((start, target) => {
    ...
    while (openSet.size > 0) {
      ...
      for (const key of openSet) {
        ...
        if (score < bestF) { bestF = score; currentKey = key; }
      }
      if (!currentKey) break;
      if (currentKey === targetKey) return reconstructPath(cameFrom, currentKey);
      ...
      for (const { dir, dx, dy } of DIR_VECTORS) {
        ...
        if (tentativeG < (gScore.get(neighborKey) ?? Infinity)) {
          ...
          fScore.set(neighborKey, tentativeG + heuristic({ x: nx, y: ny }, target));
  ...
  const stopAutoMove = useCallback((reason = null) => {
    if (autoMoveIntervalRef.current) { clearInterval(autoMoveIntervalRef.current); autoMoveIntervalRef.current = null; }
    autoPathRef.current = [];
  ...
  const startAutoMoveToCell = useCallback((targetCell) => {
    if (!shipPos) { addLog("❌ Position du bateau inconnue", "error"); return; }
    const path = findBestPath(shipPos, targetCell);
    if (!path) { addLog(`❌ Aucun chemin vers (${targetCell.x},${targetCell.y})`, "error"); setSelectedTarget(null); return; }
    if (path.length === 0) { addLog("ℹ Tu es déjà sur cette case", "info"); setSelectedTarget(targetCell); return; }
    stopAutoMove();
    ...
    addLog(`🎯 Trajet calculé vers (${targetCell.x},${targetCell.y}) | ${path.length} étape(s)`, "success");
  ...
  const deleteOffer = useCallback(async (id) => {
    ...
    if (d) { addLog(`🗑 Offre supprimée`, "success"); loadOffers(); }
  }, [api, addLog, loadOffers]);

  // ─── Thefts ──────────────────────────────────────────
  const loadThefts = useCallback(async () => {
    const d = await api("GET", "/thefts");
    if (d) { setThefts(d); addLog(`🏴☠ ${d.length} vols`, "success"); }
  }, [api, addLog]);
  ...
  const launchTheft = useCallback(async () => {
    ...
    if (d) { addLog(`🏴☠ Vol lancé: ${d.resourceType} | Chance: ${d.chance}`, "success"); loadThefts(); loadPlayerDetails(); }
  ...
  useEffect(() => {
    const h = (e) => {
      ...
      if (m[e.key]) { e.preventDefault(); stopAutoMove("⛔ Trajet auto annulé (commande manuelle)"); moveShip(m[e.key]); }
    };
  ...
  useEffect(() => {
    ...
    if (autoMoveIntervalRef.current) clearInterval(autoMoveIntervalRef.current);
    autoMoveIntervalRef.current = setInterval(async () => {
      if (loading) return;
      const nextDir = autoPathRef.current[0];
      if (!nextDir) { stopAutoMove("✅ Destination atteinte"); return; }
      const result = await moveShip(nextDir);
      if (!result) { stopAutoMove("❌ Déplacement automatique interrompu"); return; }
      autoPathRef.current = autoPathRef.current.slice(1);
      setAutoPath([...autoPathRef.current]);
      if (autoPathRef.current.length === 0) stopAutoMove("✅ Destination atteinte");
    }, 1000);
    return () => { if (autoMoveIntervalRef.current) { clearInterval(autoMoveIntervalRef.current); autoMoveIntervalRef.current = null; } };
      ...
      return (
          <div key={`${cell.x},${cell.y}`}
               ...
               style={{
                 ...
                 boxShadow: isShip ? "0 0 14px rgba(255,68,68,0.7)" : isTarget ? "0 0 0 2px rgba(78,205,196,0.9), 0 0 18px rgba(78,205,196,0.5)" : cell.type === "SAND" ? "inset 0 0 10px rgba(232,200,114,0.3)" : "none",
                 cursor: "pointer",
               }}
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 12 }}>
              ...
              {taxes.map(t => (
                  <div key={t.id} style={{ ...card, borderLeft: `3px solid ${t.state === "DUE" ? "#ff4444" : "#4ecdc4"}` }}>
                    ...
                    {t.state === "DUE" && (
                        <button onClick={() => payTax(t.id)} style={{ ...actionBtn, marginTop: 6, background: "rgba(255,68,68,0.2)", color: "#ff8888" }}>Payer cette taxe</button>
                    )}
                  </div>
              ))}
            </div>
      ...
      case "ship":
        return (
            <div style={{ flex: 1, overflowY: "auto", padding: 12 }}>
              ...
              {nextShip ? (
                  <div style={card}>
                    <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 8 }}>Niveau {nextShip.level?.id} — {nextShip.level?.name}</div>
                    ...
                    {nextShip.costResources && (
                        <div style={{ marginTop: 8 }}>
                          ...
                          {Object.entries(nextShip.costResources).map(([k, v]) => <div key={k} style={{ fontSize: 11 }}>• {k}: {v}</div>)}
                        </div>
                    )}
                    <button onClick={() => upgradeShip(nextShip.level?.id)} style={{ ...actionBtn, marginTop: 10, background: "rgba(78,205,196,0.2)" }}>⬆ Améliorer le bateau</button>
                  </div>
      ...
      case "storage":
        return (
            <div style={{ flex: 1, overflowY: "auto", padding: 12 }}>
              ...
              {nextStorage ? (
                  <div style={card}>
                    <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 8 }}>Nv.{nextStorage.id} — {nextStorage.name}</div>
                    {nextStorage.maxResources && (
                        <div style={{ marginTop: 4 }}>
                          <div style={{ fontSize: 10, color: "#5a7a9a", marginBottom: 4 }}>Capacité max:</div>
                          {Object.entries(nextStorage.maxResources).map(([k, v]) => <div key={k} style={{ fontSize: 11 }}>• {k}: {v}</div>)}
                        </div>
                    )}
                    {nextStorage.costResources && (
                        <div style={{ marginTop: 8 }}>
                          <div style={{ fontSize: 10, color: "#5a7a9a", marginBottom: 4 }}>Coût:</div>
                          {Object.entries(nextStorage.costResources).map(([k, v]) => <div key={k} style={{ fontSize: 11 }}>• {k}: {v}</div>)}
                        </div>
                    )}
                    <button onClick={upgradeStorage} style={{ ...actionBtn, marginTop: 10, background: "rgba(78,205,196,0.2)" }}>⬆ Améliorer l'entrepôt</button>
                  </div>
              ) : <div style={emptyMsg}>Clique pour voir le prochain niveau</div>}
            </div>
        );
      case "market":
        return (
            <div style={{ flex: 1, overflowY: "auto", padding: 12 }}>
              ...
              <span style={{ fontSize: 10, color: brokerConnected ? "#2ecc71" : "#ff4444" }}>
                {brokerConnected ? "🟢 Broker connecté" : "🔴 Broker déconnecté"}
              </span>
                <button onClick={() => loadOffers()} style={actionBtn}>🔄 API</button>
                <span style={{ fontSize: 10, color: "#5a6a7a" }}>{liveOffers.length} live | {offers.length} API</span>
              </div>
              <div style={{ ...card, marginTop: 4 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#4ecdc4", marginBottom: 8 }}>📤 Vendre</div>
              ...
              {liveOffers.length > 0 && (
                  <div style={{ marginTop: 10 }}>
                    <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: 2, color: "#e8c872", marginBottom: 6, fontWeight: 700 }}>⚡ Offres live (broker)</div>
                    {liveOffers.map(o => (
                        <div key={o.id} style={{ ...card, marginTop: 4, borderLeft: "3px solid #e8c872" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <div>
                              <b style={{ color: o.resourceType === "BOISIUM" ? "#8BC34A" : o.resourceType === "FERONIUM" ? "#FF9800" : "#9C27B0" }}>{o.resourceType}</b>
                              <span style={{ marginLeft: 6 }}>×{o.quantityIn}</span>
                              <span style={{ color: "#ffd700", marginLeft: 8, fontWeight: 700 }}>{o.pricePerResource} OR/u</span>
                            </div>
                            <span style={{ fontSize: 9, color: "#5a6a7a" }}>{o.owner?.name} · {o.time}</span>
                          </div>
                          <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                            <button onClick={() => quickBuy(o.id, o.quantityIn)} style={{ ...actionBtn, background: "rgba(46,204,113,0.2)", color: "#2ecc71" }}>💰 Acheter tout ({o.quantityIn})</button>
                            <button onClick={() => quickBuy(o.id, Math.min(100, o.quantityIn))} style={{ ...actionBtn, background: "rgba(78,205,196,0.2)" }}>📦 100u</button>
                            <button onClick={() => setPurchaseForm({ offerId: o.id, quantity: o.quantityIn })} style={{ ...actionBtn, fontSize: 10 }}>✏ Custom</button>
                          </div>
                        </div>
                    ))}
                  </div>
              ...
              <div style={{ ...card, marginTop: 10 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#4ecdc4", marginBottom: 8 }}>📥 Achat manuel</div>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              ...
              {offers.length > 0 && (
                  <div style={{ marginTop: 10 }}>
                    <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: 2, color: "#4ecdc4", marginBottom: 6, fontWeight: 700 }}>📋 Offres API</div>
                    {offers.map(o => (
                        <div key={o.id} style={card}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <div><b>{o.resourceType}</b> × {o.quantityIn}<span style={{ color: "#ffd700", marginLeft: 8 }}>{o.pricePerResource} OR/u</span></div>
                            <span style={{ fontSize: 10, color: "#5a6a7a" }}>{o.owner?.name}</span>
                          </div>
                          <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                            <button onClick={() => quickBuy(o.id, o.quantityIn)} style={{ ...actionBtn, fontSize: 10, background: "rgba(46,204,113,0.2)", color: "#2ecc71" }}>💰 Acheter</button>
                            <button onClick={() => deleteOffer(o.id)} style={{ ...actionBtn, fontSize: 10, color: "#ff8888" }}>🗑 Suppr</button>
                          </div>
                        </div>
                    ))}
                  </div>
              ...
              {brokerLog.length > 0 && (
                  <div style={{ marginTop: 10 }}>
                    <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: 2, color: "#5a6a7a", marginBottom: 6, fontWeight: 700 }}>📜 Events broker</div>
                    <div style={{ maxHeight: 150, overflowY: "auto", fontSize: 9, lineHeight: 1.6 }}>
                      {brokerLog.slice(-20).reverse().map((e, i) => (
                          <div key={i} style={{ padding: "2px 6px", color: e.type === "offer" ? "#e8c872" : e.type === "success" ? "#2ecc71" : e.type === "error" ? "#ff8888" : "#5a6a7a" }}>
                            <span style={{ color: "#3a4a5a", marginRight: 4 }}>{e.time}</span>{e.msg}
                          </div>
                      ))}
                    </div>
                  </div>
        ...
        return (
            <div style={{ flex: 1, overflowY: "auto", padding: 12 }}>
              ...
              <div style={{ ...card, marginTop: 8 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#4ecdc4", marginBottom: 8 }}>🏴☠ Lancer un vol</div>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  ...
                  <button onClick={launchTheft} style={{ ...actionBtn, background: "rgba(255,68,68,0.2)", color: "#ff8888" }}>Attaquer</button>
                </div>
              </div>
      ...
      case "import":
        return (
            <div style={{ flex: 1, overflowY: "auto", padding: 12 }}>
              ...
              <textarea value={importText} onChange={e => setImportText(e.target.value)} placeholder='Colle tes logs JSON ici...'
                        style={{ width: "100%", height: 200, background: "rgba(20,40,60,0.8)", border: "1px solid rgba(40,80,120,0.5)", borderRadius: 6, padding: 10, color: "#c8d8e8", fontSize: 10, fontFamily: "inherit", resize: "vertical", outline: "none" }} />
              <button onClick={doImport} style={{ ...actionBtn, marginTop: 8, width: "100%" }}>📥 Importer dans la DB</button>
              {importResult && (
                  <div style={{ ...card, marginTop: 8, color: importResult.error ? "#ff8888" : "#4ecdc4" }}>
                    {importResult.error ? `❌ ${importResult.error}` : `✅ ${importResult.imported} cellules importées (total: ${importResult.totalInDb})`}
                  </div>
              )}
            </div>
        );
  ...
  return (
      <div style={{ fontFamily: "'JetBrains Mono','Fira Code',monospace", background: "linear-gradient(135deg,#040810,#0a1628,#0d1f3c)", color: "#c8d8e8", width: "100vw", height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ background: "rgba(10,22,40,0.95)", borderBottom: "1px solid rgba(40,80,120,0.4)", padding: "8px 16px", display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
          ...
          <div style={{ background: elapsed !== null && elapsed < 3 ? "rgba(255,68,68,0.15)" : "rgba(20,40,60,0.6)", border: `1px solid ${elapsed !== null && elapsed < 3 ? "rgba(255,68,68,0.4)" : "rgba(40,80,120,0.4)"}`, borderRadius: 6, padding: "4px 12px", fontSize: 13, fontWeight: 700, minWidth: 80, textAlign: "center", color: elapsed !== null && elapsed < 3 ? "#ff8888" : "#4ecdc4" }}>
          ...
          <div style={{ width: 220, background: "rgba(8,16,30,0.95)", borderRight: "1px solid rgba(40,80,120,0.3)", padding: 12, overflowY: "auto", display: "flex", flexDirection: "column", gap: 14, flexShrink: 0 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 10, lineHeight: 2 }}>
                ...
                <div>🌊 {dbStats?.byType?.SEA || 0} | 🏖 {dbStats?.byType?.SAND || 0} | 🪨 {dbStats?.byType?.ROCKS || 0}</div>
                <div>🗺 {dbStats?.byZone?.length || 0} zones | 🧭 {dbStats?.totalMoves || 0} moves</div>
              </div>
            </div>
          </div>
          <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
            <div ref={mapRef} onMouseDown={handleMapMouseDown} onMouseMove={handleMapMouseMove} onMouseUp={handleMapMouseUp} onMouseLeave={handleMapMouseUp} onWheel={handleWheel}
              ...
              {cellsArray.length === 0 && <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", textAlign: "center", color: "#3a5a7a" }}>
                <div style={{ fontSize: 48, marginBottom: 16 }}>🗺</div><div style={{ fontSize: 14 }}>Aucune cellule</div>
              </div>}
            </div>
            ...
            {error && <div onClick={() => setError(null)} style={{ position: "absolute", bottom: 180, right: 16, background: "rgba(80,10,10,0.92)", border: "1px solid rgba(255,68,68,0.5)", borderRadius: 8, padding: "8px 14px", fontSize: 11, color: "#ff8888", maxWidth: 320, cursor: "pointer" }}>❌ {error}</div>}
            <div style={{ position: "absolute", bottom: 16, right: 16, display: "grid", gridTemplateColumns: "repeat(3,50px)", gridTemplateRows: "repeat(3,50px)", gap: 3 }}>
              {DIRS.map(({ dir, label, r, c }) => (
                  <button key={`${r}-${c}`} onClick={() => { if (!dir) return; stopAutoMove("⛔ Trajet auto annulé (commande manuelle)"); moveShip(dir); }} disabled={!dir || loading || !token}
                          style={{ gridRow: r + 1, gridColumn: c + 1, background: !dir ? "rgba(78,205,196,0.12)" : "rgba(15,30,50,0.9)", border: `1px solid ${!dir ? "rgba(78,205,196,0.3)" : "rgba(40,80,120,0.5)"}`, borderRadius: 10, color: !dir ? "#4ecdc4" : "#c8d8e8", fontSize: 22, cursor: dir && !loading && token ? "pointer" : "default", display: "flex", alignItems: "center", justifyContent: "center", opacity: !token || loading ? 0.3 : 1, fontFamily: "inherit" }}>{label}</button>
              ))}
            </div>
            <div style={{ position: "absolute", bottom: 16, left: 12, fontSize: 9, color: "#3a5a7a", background: "rgba(8,16,30,0.85)", padding: "6px 10px", borderRadius: 6, lineHeight: 1.7 }}>
              ⌨ ZQSD/flèches + A/E/W/C diag | 🖱 molette/drag
            </div>
          </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", borderBottom: "1px solid rgba(40,80,120,0.3)", flexShrink: 0 }}>
              {tabs.map(t => (
                  <button key={t.id} onClick={() => setRightTab(t.id)}
                          style={{ background: rightTab === t.id ? "rgba(78,205,196,0.1)" : "transparent", color: rightTab === t.id ? "#4ecdc4" : "#5a6a7a", border: "none", borderBottom: rightTab === t.id ? "2px solid #4ecdc4" : "2px solid transparent", padding: "8px 8px", fontSize: 10, cursor: "pointer", fontFamily: "inherit", fontWeight: 600 }}>{t.label}</button>
              ))}
            </div>
            {renderRightPanel()}
          </div>
